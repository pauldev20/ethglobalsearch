import { type Project, type SearchResponse, searchProjects } from "@/lib/api";
...
export async function ProjectsGrid({
    ...
    types,
    organizations,
}: { query?: string; page?: number; events?: string; types?: string; organizations?: string }) {
    let data = null;
    let pagination = null;
    if (
        query !== undefined &&
        ...
        <>
            ...
            {pagination && data && data.length > 0 && (
                <div className="mb-6">
                    <span className="text-sm text-muted-foreground">
                        ...
                        of <span className="font-semibold text-foreground">{totalDisplay}</span> projects
                    </span>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {data === null &&
                    ...
                    Array.from({ length: 6 }).map((_, index) => <ProjectCard key={index} project={undefined} />)}
                {(data === undefined || data?.length === 0) && (
                    <div className="col-span-full flex flex-col items-center justify-center py-16 px-4">
                        <div className="text-6xl mb-4">🔍</div>
                        <h3 className="text-xl font-semibold mb-2">No projects found</h3>
                        <p className="text-muted-foreground max-w-md text-center">
                            We searched everywhere but couldn't find any projects matching your criteria. Try adjusting
