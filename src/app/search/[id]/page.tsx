...
import { type Project, getProject } from "@/lib/api";
        ...
        <main className="min-h-screen w-full pb-8">
            <section className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-6 space-y-8">
                ...
                <div className="space-y-4">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <p className="mt-0.5 text-sm text-muted-foreground">{project.event_name}</p>
                            </div>
                        </div>
                        ...
                        {(project.url || project.source_code_url) && (
                            <div className="hidden sm:flex gap-3 shrink-0">
                                {project.url && (
                                    <Button asChild={true} size="default" className="gap-2">
                                ...
                                {project.source_code_url && (
                                    <Button asChild={true} size="default" variant="outline" className="gap-2">
                    ...
                    <p className="text-sm leading-relaxed text-muted-foreground sm:text-base">{project.tagline}</p>

                    {project.prizes.length > 0 && (
                        <div className="flex flex-wrap items-center gap-2">
                            {project.prizes.map((prize) => (
                                <Badge key={prize.prize_name} variant="default" className="gap-1 px-2 py-1 text-xs">
                                    {prize.prize_emoji ? (
                                        <span>{prize.prize_emoji}</span>
                    ...
                    {(project.url || project.source_code_url) && (
                        <div className="flex flex-col gap-2 sm:hidden">
                            {project.url && (
                                <Button asChild={true} size="lg" className="w-full gap-2">
                                    <a href={project.url} target="_blank" rel="noopener noreferrer">
                                        <FontAwesomeIcon icon={faGlobe} className="h-4 w-4" />
                                        <span>Live Demo</span>
                                    </a>
                            ...
                            {project.source_code_url && (
                                <Button asChild={true} size="lg" variant="outline" className="w-full gap-2">
                                    <a href={project.source_code_url} target="_blank" rel="noopener noreferrer">
                                        <FontAwesomeIcon icon={faGithub} className="h-4 w-4" />
                                        <span>Source Code</span>
                                    </a>
                ...
                <div className="space-y-8">
                    <Card className="border-2 p-6 sm:p-8">
                        <h2 className="mb-4 text-xl font-bold text-foreground sm:text-2xl">About this project</h2>
                        <RenderedText text={project.description} />
                    </Card>

                    {project.how_its_made && (
                        <Card className="border-2 p-6 sm:p-8">
                            <h2 className="mb-4 text-xl font-bold text-foreground sm:text-2xl">How it&apos;s Made</h2>
