...
export function ProjectCard({ project, highlights, priority = false }: { project?: Project; highlights?: { [key: string]: any }; priority?: boolean }) {
    if (!project) {
        return (
            <Card className="relative overflow-hidden border-2 bg-card h-full animate-pulse p-0 flex flex-col gap-0">
                {/* Location badge placeholder — matches Badge size (h-5 w/ px-2 py-0.5) */}
                <div className="absolute top-0 left-0 z-10 m-2 h-5 w-32 bg-muted rounded-full" />

                {/* Skeleton Image */}
                <div className="aspect-video w-full bg-muted" />

                {/* Skeleton Info — same padding + line heights as final */}
                <div className="p-4 flex flex-col flex-1">
                    {/* Title row matches h3 text-xl (line-height ~1.75rem = 28px) */}
                    <div className="flex items-center justify-between gap-2 mb-3 h-7">
                        <div className="h-6 bg-muted rounded w-3/4" />
                    </div>

                    {/* Tagline: 2 lines text-sm leading-relaxed → ~2.875rem total */}
                    <div className="mb-3 h-[2.875rem] flex flex-col justify-between">
                    ...
                    <div className="pt-2 border-t border-border/40 h-[38px] flex items-center gap-1.5">
